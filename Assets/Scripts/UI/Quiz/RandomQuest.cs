...
public class RandomQuest : MonoBehaviour
{
    ...
    public void OnLevelButtonClicked(Button clickedButton)
    {
        ...
        BtnSoundManager.Instance.PlayButtonSound();  // 레벨 버튼 클릭 시 사운드 추가
    ...
    IEnumerator ShowRightPanel()
    {
        if (rightPanel != null)
        {
            ...
            BtnSoundManager.Instance.PlayButtonSound();  // // 정답 패널 활성화 시 사운드 추가
    ...
    IEnumerator ShowWrongPanel(bool moveToNext)
    {
        if (wrongPanel != null)
        {
            ...
            BtnSoundManager.Instance.PlayButtonSound();  // 오답 패널 활성화 시 사운드 추가
            yield return new WaitForSecondsRealtime(1.5f);
            ...
            else
            {
                ...
                {
                    button.onClick.AddListener(() => { CheckAnswer(System.Array.IndexOf(answerButton, button) + 1); BtnSoundManager.Instance.PlayButtonSound(); });
                }
                countdownTimer.StartTimer(TimerDuration(), OnTimeOut);
            }
        }
